...
export const getPlatformsApi = async (
  ...
  const response = await api.get<{ data: Platform[] }>(
    ENDPOINTS.PLATFORMS.BASE,
    {
      params: {
        ...params,
...
export const createPlatformApi = async (data: {
  platform_name: string;
  color_key?: string | null;
}): Promise<Platform> => {
  const response = await api.post<{ data: Platform }>(
    ENDPOINTS.PLATFORMS.BASE,
    data,
  );
  return response.data.data;
...
export const updatePlatformApi = async (
  ...
  const response = await api.put<{ data: Platform }>(
    ENDPOINTS.PLATFORMS.DETAIL(id),
    data,
  );
  return response.data.data;
};
