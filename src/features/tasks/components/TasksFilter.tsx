              ...
              <TabsList className="bg-gray-200/80 p-1 rounded-xl h-10 gap-1 w-full sm:w-auto justify-start flex-nowrap min-w-max">
                {typeFilters.map((filter) => (
                  <TabsTrigger
                    key={filter.key}
                    value={filter.key}
                    className="rounded-lg text-xs font-semibold px-3 py-1.5 data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-red-logo flex items-center gap-1.5 cursor-pointer"
                  >
