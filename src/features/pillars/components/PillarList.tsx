...
export function PillarList({ pillars, onEdit, onDelete }: PillarListProps) {
  if (pillars.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center text-center py-12 px-4 bg-gray-50/30 rounded-2xl border border-dashed border-gray-200 flex-1">
        ...
        <p className="text-xs text-gray-400 max-w-xs leading-relaxed font-semibold">
          Start by creating a new content pillar to map your content topic
          focus.
        </p>
      </div>
    );
        ...
        <TableBody>
          {pillars.map((pillar) => (
            ...
            >
              ...
              <TableCell className="py-3.5 text-gray-500 text-sm max-w-64 truncate">
                {pillar.description || "-"}
              </TableCell>
