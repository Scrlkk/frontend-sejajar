...
export function ContentCategoryFormModal({
  isOpen,
  onClose,
  editingCategory,
  onSave,
  ...
  const [categoryName, setCategoryName] = useState(editingCategory?.type_name ?? "");
  const [categoryColor, setCategoryColor] = useState<string | null>(editingCategory?.color_key ?? null);
  const [categoryActive, setCategoryActive] = useState(editingCategory?.is_active ?? true);
      ...
      <DialogContent className="sm:max-w-md bg-white border border-gray-100 shadow-2xl rounded-2xl p-6 outline-none">
        <DialogHeader className="border-b border-gray-100 pb-3 flex flex-col gap-1 text-left">
          <DialogTitle className="text-base font-bold text-gray-900 leading-none">
            {mode === "create" ? "Add Content Category" : "Edit Content Category"}
          </DialogTitle>
          <p className="text-xs font-semibold text-gray-400 mt-0.5 leading-normal font-sans">
            {mode === "create"
              ? "Create a new content category/type to organize contents."
              : "Modify the selected content category type name."}
          </p>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-3 text-left">
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="e.g. Video, Image, Carousel"
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              ...
              {PALETTE_KEYS.map((colorKey) => {
                ...
                const isSelected = categoryColor === colorKey;
                return (
                  <button
                    key={colorKey}
                    type="button"
                    onClick={() => setCategoryColor(colorKey)}
